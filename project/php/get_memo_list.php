<?php
	...
	if(mysqli_connect_errno($conn)){
		...
		$sql = "SELECT * FROM ssm_memo WHERE user_id = '$user_id' AND date >= '$now_date' ORDER BY date ASC LIMIT $startNum , $blockCnt";
		...
		if($result){
			...
			while($row = mysqli_fetch_array($result)){
				...
				$music_info['date'] = $row[date];
				$music_info['created'] = $row[created];

				$music_list[] = $music_info;
			}
			$json_result['memo_list'] = $music_list;
